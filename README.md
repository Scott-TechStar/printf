...
A custom _printf() for learning purposes was developed by cohort  #13 students John Mwadime and OYADIER MENSAH.
...
|`print_37.c`|Contains percentage print function.|`None` |
|`print_int.c` | Contains decimal and integer functions. | `None` | 
`print_char.c` | Custom function for char data type. | `None`
|`print_str.c` | Function that calls string type variable. | `None` |
|`print_bin.c` | Function that gets the binary | `None` |
`print_oct.c` | Functions that returns octal number. | `None` |
...
`print_hexg.c` | Auxiliar function for hexadecimal specific functions (lowercase). | `print_strn.c` | 
`print_HEXg.c` | Auxiliar function hexadecimal specific functions (Uppercase). | `print_p.c` |
`print_strn.c` | Returns a string and non readable characters are printed in hexadecimal numbers (Lowercase). | `print_hexg.c` |
`print_srev.c` | Returns a string in reverse. | `None` |
`print_rot13.c` | Returns a string in Rot13. | `None` |
`print_str.c` | Auxiliar functions such as strlen and strcpy. | `None` |
